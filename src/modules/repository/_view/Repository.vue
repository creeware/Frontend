<template>
    <v-layout>
        <v-flex xs6 class="ma-1">
            <repository-card v-if="orgLoaded&&userLoaded" :repository="repository" :color="repositoryColor" @handle-delete="handleDelete"/>
            <user-card v-if="userLoaded&&orgLoaded" :user="user" :color="userColor" />
            <organization-card v-if="orgLoaded&&userLoaded" :organization="organization" :color="organizationColor" />
        </v-flex>
...
/*                 repository: {
 ...
 /*                organization: {
        ...
        beforeRouteEnter(to, from, next){
            store.dispatch("getRepository", to.params.uuid).then(() =>
            next())
        },
        mounted(){
            store.dispatch("getOrganization", this.repository.organization_uuid).then(() => this.$data.orgLoaded = true)
            store.dispatch("getUser", this.repository.user_uuid).then(() => this.$data.userLoaded = true)
        ...
        methods: {
            ...
            checkIfDue(date){
                ...
                if(date.getFullYear()==today.getFullYear() && date.getMonth()==today.getMonth() && date.getDate()==today.getDate()){
                    this.snackbarMessage = "This "+this.repository.repository_type+" is due for today!"
                    this.snackbarColor = "warning"
                    this.snackbar = true
                } else if(today > date){
                    this.snackbarMessage = "This "+this.repository.repository_type+" has expired!"
                    this.snackbarColor = "error"
                    this.snackbar = true
                } else if(daysLeft <= 30){                    
                    this.snackbarMessage = "This "+this.repository.repository_type+" is due in "+daysLeft+" days!"
